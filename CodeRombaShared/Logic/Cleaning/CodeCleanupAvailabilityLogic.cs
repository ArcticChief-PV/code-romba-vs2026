...
namespace WinterbiteStudios.CodeRomba.Logic.Cleaning
{
    ...
    internal class CodeCleanupAvailabilityLogic
    {
        ...
        internal static CodeCleanupAvailabilityLogic GetInstance(CodeRombaPackage package)
        {
        ...
        /// </summary>
        /// <param name="package">The hosting package.</param>
        private CodeCleanupAvailabilityLogic(CodeRombaPackage package)
        {
            _package = package;
        }
        ...
        internal bool CanCleanupDocument(Document document, bool allowUserPrompts = false)
        {
            ...
            {
                OutputWindowHelper.DiagnosticWriteLine($"CodeCleanupAvailabilityLogic.CanCleanupDocument returned false for '{document.FullName}' due to the document language not being included within CodeRomba Options.");
                return false;
            }
            ...
            {
                OutputWindowHelper.DiagnosticWriteLine($"CodeCleanupAvailabilityLogic.CanCleanupDocument returned false for '{document.FullName}' due to the file name being excluded within CodeRomba Options.");
                return false;
            }

            if (!IsFileNameIncludedByOptions(document.FullName))
            {
                OutputWindowHelper.DiagnosticWriteLine($"CodeCleanupAvailabilityLogic.CanCleanupDocument returned false for '{document.FullName}' due to the file name not being included within CodeRomba Options.");
                return false;
            }
        ...
        internal bool CanCleanupProjectItem(ProjectItem projectItem)
        {
            ...
            {
                OutputWindowHelper.DiagnosticWriteLine($"CodeCleanupAvailabilityLogic.CanCleanupProjectItem returned false for '{projectItemFileName}' due to the project item language not being included within CodeRomba Options.");
                return false;
            }

            if (IsFileNameExcludedByOptions(projectItemFileName))
            {
                OutputWindowHelper.DiagnosticWriteLine($"CodeCleanupAvailabilityLogic.CanCleanupProjectItem returned false for '{projectItemFileName}' due to the file name being excluded within CodeRomba Options.");
                return false;
            }
        ...
        private static bool PromptUserAboutCleaningExternalFiles(Document document)
        {
            try
            {
                ...
                {
                    Title = Resources.CodeRombaCleanupExternalFile,
